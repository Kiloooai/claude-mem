...
class Store {
  ...
  /**
   * Get daily session counts for the past N days (including today).
   * @param {number} days
  ...
  getDailyCounts(days = 7) {
    ...
    for (let d = new Date(cutoff); d <= today; d.setDate(d.getDate() + 1)) {
      counts[fmt(new Date(d))] = 0;
    }

    // Aggregate from index
    for (const meta of Object.values(index)) {
      if (meta.date >= cutoffStr && meta.date <= todayStr) {
        counts[meta.date] = (counts[meta.date] || 0) + 1;
      }
    }
